...
public class VarausGUI extends Application {
    ...
    private Kayttaja valittuAsiakas;
    private Varaus valittuVaraus;
    private Lasku valittuLasku;
    private Mokki valittuMokki;

    // Mökki-oliot testaamiseen
    Mokki mokki1 = new Mokki(0,String.valueOf(2), "Kanada", 10,false, 200);
    Mokki mokki2 = new Mokki(1,String.valueOf(1),"Turku", 15, true, 400);
    Mokki mokki3 = new Mokki(2,String.valueOf(0), "Suomi", 20, false, 600);

    // Kayttaja-oliot testaamiseen
    Kayttaja kayttaja1 = new Kayttaja("Jussi", "Jokinen",0, "jussi.jokinen@example.com");
    Kayttaja kayttaja2 = new Kayttaja("Matti", "Meikäläinen",1, "matti.meikalainen@example.com");
    Kayttaja kayttaja3 = new Kayttaja("Risto", "Rauta",2, "risto.rauta@example.com");

    // Varaus-oliot testaamiseen
    Varaus varaus1 = new Varaus(0, mokki1, kayttaja1, Date.valueOf(LocalDate.now()), Date.valueOf(LocalDate.now().plusDays(3)), 200.0);
    Varaus varaus2 = new Varaus(1, mokki2, kayttaja2, Date.valueOf(LocalDate.now()), Date.valueOf(LocalDate.now().plusDays(5)), 400.0);
    Varaus varaus3 = new Varaus(2, mokki3, kayttaja3, Date.valueOf(LocalDate.now()), Date.valueOf(LocalDate.now().plusDays(7)), 600.0);

    // Lasku-oliot testaamiseen
    Lasku lasku1 = new Lasku(kayttaja1, varaus1,false,Date.valueOf(LocalDate.now()), Date.valueOf(LocalDate.now().plusDays(10)),varaus1.getHinta());
    Lasku lasku2 = new Lasku(kayttaja2, varaus2,false,Date.valueOf(LocalDate.now()), Date.valueOf(LocalDate.now().plusDays(13)),varaus2.getHinta());
    Lasku lasku3 = new Lasku(kayttaja3, varaus3,true,Date.valueOf(LocalDate.now()), Date.valueOf(LocalDate.now().plusDays(1)),varaus3.getHinta());
            ...
            if (node instanceof TextField textField) {
                switch (i) {
                    case 0:
                        ...
                        break;
                    case 1:
                        textField.setText("Omistaja");
                        break;
                    case 2:
                        textField.setText("Osoite");
                        break;
                    case 3:
                        textField.setText(String.valueOf(mokki.getMaksimiAsukkaat()));
                        break;
                    case 4:
    ...
    private void luoMokkiLista() {
        ...
        for (Mokki mokki : mokkiLista) {
            Button text = new Button(String.valueOf(mokki.getID()));
            text.setOnAction(e -> {mokinTiedotKenttiin(mokki);});
    ...
    private void luoLaskuLista() {
        ...
        for (Lasku lasku : laskuLista) {
            ...
            text.setOnAction(e -> {laskunTiedotKenttiin(lasku);});
            vbox.getChildren().add(text);
            text.setPrefWidth(195);
        }
        scrollPane.setContent(vbox);
    ...
    private void luoVarausLista() {
        ...
        for (Varaus varaus : varausLista) {
            ...
            text.setOnAction(e -> {varauksenTiedotKenttiin(varaus);});
            vbox.getChildren().add(text);
            text.setPrefWidth(195);
        }
        scrollPane.setContent(vbox);
    ...
    private void luoAsiakasLista() {
        VBox vbox = new VBox();
        for (Kayttaja asiakas : asiakasLista) {
            Button text = new Button(asiakas.getSukunimi() +" "+ asiakas.getEtunimi());
            text.setOnAction(e -> {
                valittuAsiakas = asiakas;
                asiakkaanTiedotKenttiin(asiakas);
            });
            vbox.getChildren().add(text);
            text.setPrefWidth(195);
        }
    ...
    private Mokki lueMokinTiedot(Nakymatyyppi nakymatyyppi, GridPane kentat) {
        Mokki mokki = null;
        switch (nakymatyyppi) {
            case MOKIT: {
                int i = 0;
                    ...
                    if (node instanceof TextField textField) {
                        switch (i) {
                            case 0:
                                ...
                                break;
                            case 1:
                                mokki.setOmistajaID(textField.getText());
                                break;
                            case 2:
                                textField.setText("Osoite");
                                break;
